...
**Example Usage (Displaying a Sensor Value):**
```yaml
# This service call will put "Temp: 21.5°C" on the top row and leave it there.
...
- service: script.bttf_time_circuits_universal_effect_generator
  data:
    ...
    prefix: "Temp: "
    postfix: "°C"
    restore_row: false # Leave the text on the display
```

**Example Usage (Marquee):**
```yaml
# This service call will scroll a long message on the middle row.
- service: script.bttf_time_circuits_universal_effect_generator
  data:
    target_row: "MIDDLE"
    effect: "MARQUEE"
...
```yaml
# This service call will run a 10-second countdown on the bottom row.
- service: script.bttf_time_circuits_universal_effect_generator
  data:
    target_row: "BOTTOM"
    effect: "COUNTDOWN"
