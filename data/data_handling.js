...
/**
 * Handles the data zip file upload.
 * @param {Event} event The submit event from the data zip upload form.
 */
function handleDataZipUpload(event) {
    event.preventDefault();
    const form = event.target;
    const fileInput = form.querySelector('input[type="file"]');
    const file = fileInput.files[0];
    if (!file) {
        showMessage('Please select a data.zip file to upload.', 'error');
        return;
    }
    const formData = new FormData();
    formData.append('data-zip', file, file.name);

    const progressBar = document.getElementById('data-zip-progress-bar');
    const statusMessage = document.getElementById('data-zip-status-message');
    progressBar.style.width = '0%';
    statusMessage.textContent = 'Uploading and extracting data.zip...';

    const xhr = new XMLHttpRequest();
    xhr.open('POST', '/upload-data-zip', true);
    ...
    xhr.onload = function() {
        if (xhr.status === 200) {
            statusMessage.textContent = 'Data zip uploaded and extracted successfully!';
            showMessage('Data zip uploaded and extracted successfully!', 'success');
        } else {
            statusMessage.textContent = 'Data zip upload failed.';
            showMessage('Data zip upload failed.', 'error');
        }
    };

    xhr.onerror = function() {
        statusMessage.textContent = 'Data zip upload error.';
        showMessage('Data zip upload error.', 'error');
    };
