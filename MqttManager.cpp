...
void handleSequencerCommand(const std::string& payload) {
    ...
    if (payload == "Intruder Alert") {
        ...
        json_to_parse = R"([
            {"targetRow":1, "commands":[{"command":"COUNTDOWN", "stringParam":"LAUNCH IN", "intParam":10, "intParam2":1000}, {"command":"MARQUEE", "stringParam":"LIFTOFF!"}, {"command":"SOUND", "stringParam":"engine_rev.mp3"}]}
        ])";
