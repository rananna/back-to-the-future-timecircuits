...
> **Note on availability:** While most animations can be triggered by name, a few older or highly specific effects can only be started using the `TRIGGER_ANIMATION` command with the appropriate numeric `AnimationType` ID. All major, thematic animations are available via the names listed below.
...
| `Scanner` | A Cylon-style red scanner that sweeps back and forth across all three display rows simultaneously. |
| `Flux Capacitor Overload` | All three display rows pulse with energy in a chaotic, asynchronous pattern, simulating an overloaded Flux Capacitor. |
| `System Error` | A two-row error message with "ERROR" scrambling on the top row and "SYSTEM MALFUNCTION" scrolling on the middle row. |
