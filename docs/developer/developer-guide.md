...
| :--- | :--- |
| `All Displays Random` | The classic BTTF effect. All three rows simultaneously scramble and lock in the current time over 10 seconds. |
...
| `Code Breaker` | A slower, more deliberate version of the scramble & lock-in effect. |
| `Countdown` | A 10-second countdown on the middle row, spelling out the numbers, and ending with a "LIFTOFF!" marquee. |
| `Counting Up` | All three displays rapidly flicker with random numbers. |
...
These animations are defined as hardcoded JSON strings in `AnimationSequences.cpp`. Their names in the Web UI are defined in `data/sequences.json`.
