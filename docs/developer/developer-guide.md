...
*   **Web Server & UI**: `web_server.cpp` serves the frontend files from the `data/` directory and provides the backend REST API and a WebSocket server that the web UI communicates with.

---

## ⚙️ Advanced Configuration

This section provides a reference for all major configuration options, including firmware settings (compile-time) and web UI settings.
