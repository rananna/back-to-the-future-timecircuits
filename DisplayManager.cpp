...
char manualDisplayText[3][4][MAX_MANUAL_TEXT_LENGTH];
...
void updateDisplaySegment(int row, int segment, const char* text) {
    ...
    if (segment == -1) {
        ...
        strncpy(manualDisplayText[row][segment], text, MAX_MANUAL_TEXT_LENGTH - 1);
        manualDisplayText[row][segment][MAX_MANUAL_TEXT_LENGTH - 1] = '\0';

        // A row is in manual mode if any of its segments have text.
        isRowInManualMode[row] = (manualDisplayText[row][0][0] != '\0') ||
                                 (manualDisplayText[row][1][0] != '\0') ||
                                 (manualDisplayText[row][2][0] != '\0') ||
                                 (manualDisplayText[row][3][0] != '\0');
    } else {
